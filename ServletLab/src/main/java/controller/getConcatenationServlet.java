package controller;

import java.io.IOException;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import model.NumberAdder;

/**
 * Servlet implementation class getConcatenationServlet
 */
@WebServlet("/getConcatenationServlet")
public class getConcatenationServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public getConcatenationServlet() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		String numOne = request.getParameter("userNumberOneC");
		String numTwo = request.getParameter("userNumberTwoC");
		
		NumberAdder result = new NumberAdder(Integer.parseInt(numOne), Integer.parseInt(numTwo));
		
		//uRes = UserResults. Much easier to read html with abbreviated variable
		request.setAttribute("uRes", result);
		
		getServletContext().getRequestDispatcher("/resultTwo.jsp").forward(request, response);
	}

}
